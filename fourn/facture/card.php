...
require_once DOL_DOCUMENT_ROOT.'/basic_lib/pedido_proveedor.php';
	...
	elseif ($action == 'add' && $user->rights->fournisseur->facture->creer)
	{
		...
		else
		{
			$db->commit();
                        ...
                        //VASA si es te que crear la factura:bouton
                        if($_POST['bouton'] == $langs->trans('CreateDraft')){
                            $pedidoProveedor = new PedidoProveedor($db);
                            $pedidoProveedor->marcarPedidoProvFacturat($_POST['originid']);
                        }
...
if ($action == 'create')
{
	...
	else
	{
        ...
        //VASA si tenim el proveedor omplim la referencia automatica
        if($societe->id > 0 && $objectsrc->ref_supplier == ""){
            $objectsrc->ref_supplier = $societe->code_fournisseur."_".strtotime("now");
        }

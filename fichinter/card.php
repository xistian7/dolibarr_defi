...
$intervencio = new Intervencio($db);
...
if (empty($reshook))
{
	...
	}elseif ($action == 'reopenintervencio' && $user->rights->ficheinter->creer)
	{
	    $result = $object->setStatut(1);
	    if ($result > 0)
	    {
	...
	if ($socid > 0)
	{
		...
		if (!empty($conf->projet->enabled))
		{
			$formproject = new FormProjets($db);
                        ...
                        if($projectid == NULL){
                            $projectid = $intervencio->getDefaultProject();
                        }
			$langs->load("project");
		...
		// Contract VASA no fem servir contractes
		/*if ($conf->contrat->enabled)
		{
...
elseif ($id > 0 || !empty($ref))
{
	...
	print '</tr>';
        ...
        //VASA de moment no usen contractes
	// Contract
	/*if ($conf->contrat->enabled)
	{
		$langs->load('contracts');
 	...
 	if (empty($conf->global->FICHINTER_DISABLE_DETAILS))
 	{
		...
		if ($resql)
		{
			...
			if ($num)
			{
				...
				print '<td class=" col-md-2 col-md-first">'.$langs->trans('Producto/Servicio').'</td>';
                                print '<td class=" col-md-1 col-md-first" style="text-align:center;">'.$langs->trans('Cantidad').'</td>';
                                print '<td class=" col-md-1 col-md-first" style="text-align:center;">'.$langs->trans('Descuento (%)').'</td>';
                                print '<td class=" col-md-5 col-md-first">'.$langs->trans('Description').'</td>';
				print '<td class=" col-md-1 col-md-first" style="text-align:center;">'.$langs->trans('Date').'</td>';
				//print '<td class=" col-md-1 col-md-first">'.(empty($conf->global->FICHINTER_WITHOUT_DURATION) ? $langs->trans('Duration') : '').'</td>';
				print '<td class=" col-md-1 col-md-first">&nbsp;</td>';
				print '<td class=" col-md-1 col-md-first">&nbsp;</td>';
				print '<td class=" col-md-1 col-md-first">&nbsp;</td>';
				print "</tr>\n";
			}
			while ($i < $num)
			{
				...
				if ($action != 'editline' || GETPOST('line_id', 'int') != $objp->rowid)
				{
					...
					// Date
					print '<td class="col-md-1 col-md-first" style="text-align:center;">'.(empty($conf->global->FICHINTER_DATE_WITHOUT_HOUR) ?dol_print_date($db->jdate($objp->date_intervention), 'day') : dol_print_date($db->jdate($objp->date_intervention), 'day')).'</td>';

					// Duration
					//print '<td class="col-md-1 col-md-first">'.(empty($conf->global->FICHINTER_WITHOUT_DURATION) ?convertSecondToTime($objp->duree) : '').'</td>';
					...
					{
						print '<td class=" col-md-1 col-md-first">';
					...
					{
						print '<td class=" col-md-3 col-md-first">&nbsp;</td>';
					}
			...
			if ($object->statut == 0 && $user->rights->ficheinter->creer && $action <> 'editline' && empty($conf->global->FICHINTER_DISABLE_DETAILS))
			{
				if (!$num)
				{
    				...
    				print '<tr class="liste_titre">';
                                print '<td  class="col-md-2 col-md-first">'.$langs->trans('Producto/Servicio').'</td>';
                                print '<td  class="col-md-1 col-md-first">'.$langs->trans('Cantidad').'</td>';
                                print '<td  class="col-md-1 col-md-first">'.$langs->trans('Descuento (%)').'</td>';
    				print '<td  class="col-md-4 col-md-first">';
    				print '<a name="add"></a>'; // ancre
    				print $langs->trans('Description').'</td>';
    				print '<td class=" col-md-1 col-md-first">'.$langs->trans('Date').'</td>';
    				//print '<td class=" col-md-1 col-md-first">'.(empty($conf->global->FICHINTER_WITHOUT_DURATION) ? $langs->trans('Duration') : '').'</td>';
      				print '<td class=" col-md-3 col-md-first">&nbsp;</td>';
    				print "</tr>\n";
				}
                ...
                if (empty($conf->global->FICHINTER_EMPTY_LINE_DESC)) {
                    require_once DOL_DOCUMENT_ROOT.'/core/class/doleditor.class.php';
                    $doleditor = new DolEditor('np_desc', GETPOST('np_desc', 'alpha'), '', 50, 'dolibarr_details', '', false, true, $conf->global->FCKEDITOR_ENABLE_DETAILS, ROWS_2, '100%');
                    $doleditor->Create();
                }
                print '</td>';
                //print '<td>';
                //print '<td class=" col-md-1 col-md-first"><input type="text" class="flat maxwidth75 maxwidthonsmartphone" name="options_cantidad2" id="options_cantidad2" value=""></div>';
                // Date intervention
                print '<td class=" col-md-1 col-md-first">';
				$now = dol_now();
                ...
                print '</td>';*/
                print '<input type="hidden" id="duration" name="durationhour" value="1">';
                print '<td class=" col-md-3 col-md-first" valign="middle" colspan="3"><input type="submit" class="button" value="'.$langs->trans('Add').'" name="addline"></td>';
				print '</tr>';
            ...
            if ($action != 'editdescription' && ($action != 'presend')) {
                ...
                if ($object->statut == Fichinter::STATUS_VALIDATED && ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && $user->rights->ficheinter->creer) || (!empty($conf->global->MAIN_USE_ADVANCED_PERMS) && $user->rights->ficheinter->ficheinter_advance->unvalidate)))
				{
					...
					print '</a></div>';
                                ...
                                //VASA de doment no envieam mails
				// Send
				...
				{
					$langs->load("bills");
					if ($object->statut < Fichinter::STATUS_BILLED || $object->statut == Fichinter::STATUS_CLOSED)
					{
				...
				// Done
            	if ((empty($conf->global->FICHINTER_CLASSIFY_BILLED) && $object->statut > Fichinter::STATUS_DRAFT && $object->statut < Fichinter::STATUS_CLOSED) && $object->statut != Fichinter::STATUS_BILLED)
				{

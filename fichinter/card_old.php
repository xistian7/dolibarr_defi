...
elseif ($id > 0 || ! empty($ref))
{
    ...
    if (empty($conf->global->FICHINTER_DISABLE_DETAILS))
    {
        ...
        print '</tr>';
		print '<tr><td class="titlefield">Import intervenció</td>';
		/*print '<td>'.round((53.38/3600)*$object->duration,2).' €</td>';
		print '</tr>';
		$objectsrc->total_ht=round((53.38/3600)*$object->duration,2);
		$object->total_ht=999;*/
//		print_r($object);
 	...
 	if (empty($conf->global->FICHINTER_DISABLE_DETAILS))
 	{
		...
		if ($resql)
		{
			...
			if ($num)
			{
				...
				print '<td class="liste_titre">Import</td>';
...
//					print '<div class="inline-block divButAction"><a class="butAction" href="' . $_SERVER['PHP_SELF'] . '?id=' . $object->id . '&amp;socid=' . $object->socid . '&amp;action=clone&amp;object=ficheinter">' . $langs->trans("ToClone") . '</a></div>';
				...
				// Delete
				if ((($object->statut == Fichinter::STATUS_DRAFT && $user->rights->ficheinter->creer) || $user->rights->ficheinter->supprimer) && $object->statut < Fichinter::STATUS_BILLED)
				{

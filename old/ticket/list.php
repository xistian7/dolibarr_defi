...
foreach($search as $key => $val)
{
	...
	{
	    $tmpstatus='';
	    if ($search['fk_statut'] == 'openall' || in_array('openall', $search['fk_statut'])) $tmpstatus.=($tmpstatus?',':'')."'0', '1', '2', '3', '4', '5', '6'";
...
while ($i < min($num, $limit))
{
	...
	foreach($object->fields as $key => $val)
	{
		...
		if (! empty($arrayfields['t.'.$key]['checked']))
		{
			$timesince=ceil((time()-$db->jdate($obj->$key))/3600);
			if ($timesince>255) $timesince=255;
			$color=hexdec("FF")-$timesince;
			$colorfromopened="ff".str_pad(dechex($color), 2, "0", STR_PAD_LEFT).str_pad(dechex($color), 2, "0", STR_PAD_LEFT);
			print '<td';
			...
			print $val['css'];
			if ($cssforfield || $val['css']) print '"';
			if (in_array($val['type'], array('date','datetime','timestamp')) && $db->jdate($obj->$key)<>0) print "style='background-color:#$colorfromopened'";
			print '>';

...
function print_start_menu_entry($idsel, $classname, $showmode)
{
	if ($showmode)
	{
            ...
            if ($showmode == 1)
            {
                ...
                $classname = $classname.' disabledMenu"';
                print '<li '.$classname.' id="mainmenutd_'.$idsel.'">';
            }
	...
	if (empty($noout))
	{
		...
		for ($i = 0; $i < $num; $i++)     // Loop on each menu entry
		{
			...
			if ($menu_array[$i]['level'] > 0  || $menu_array[$i]['mainmenu'] == 'home')
			{
				...
				{
					print '<div class="disabledMenu menu_contenu'.$cssmenu.'">'.$tabstring.'<font class="vsmenudisabled vsmenudisabledmargin">'.$menu_array[$i]['titre'].'</font><br></div>'."\n";
				}
			}
